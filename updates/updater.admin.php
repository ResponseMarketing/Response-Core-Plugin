 ...
 *
 * @see /wp-admin/plugins.php?page=resp_github-updater
 */
class WPGitHubUpdaterSetup {
	...
	function __construct() {
		...
		add_action( 'wp_ajax_set_resp_github_oauth_key', array( $this, 'ajax_set_resp_github_oauth_key' ) );
	}
	...
	function add_page() {
		if ( current_user_can ( 'manage_options' ) ) {
			$this->options_page_hookname = add_plugins_page ( __( 'Response Github Updates', 'resp_github_plugin_updater' ), __( 'Response Github Updates', 'resp_github_plugin_updater' ), 'manage_options', 'resp_github-updater', array( $this, 'admin_page' ) );
	...
	public function settings_fields() {
		register_setting( 'respghupdate', 'respghupdate', array( $this, 'settings_validate' ) );

		// Sections: ID, Label, Description callback, Page ID
		add_settings_section( 'respghupdate_private', 'Private Repositories', array( $this, 'private_description' ), 'resp_github-updater' );

		// Private Repo Fields: ID, Label, Display callback, Menu page slug, Form section, callback arguements
		add_settings_field(
			'client_id', 'Client ID', array( $this, 'input_field' ), 'resp_github-updater', 'respghupdate_private',
			array(
				'id' => 'client_id',
				'type' => 'text',
				'description' => '',
		...
		add_settings_field(
			'client_secret', 'Client Secret', array( $this, 'input_field' ), 'resp_github-updater', 'respghupdate_private',
			array(
				'id' => 'client_secret',
				'type' => 'text',
				'description' => '',
		...
		add_settings_field(
			'access_token', 'Access Token', array( $this, 'token_field' ), 'resp_github-updater', 'respghupdate_private',
			array(
				'id' => 'access_token',
			)
		);
		add_settings_field(
			'gh_authorize', '<p class="submit"><input class="button-primary" type="submit" value="'.__( 'Authorize with Response Github', 'resp_github_plugin_updater' ).'" /></p>', null, 'resp_github-updater', 'respghupdate_private', null
		);
...
?>
		...
		<ol>
			...
			<li><a href="javascript:document.forms['respghupdate'].submit();">Authorize with Response Github</a>.</li>
		</ol>
		<?php
	}
...
?>
	...
	public function token_field( $args ) {
		...
		if ( empty( $value ) ) {
?>
			<p>Input Client ID and Client Secret, then <a href="javascript:document.forms['respghupdate'].submit();">Authorize with Response Github</a>.</p>
...
?>
	...
	public function settings_validate( $input ) {
		...
		if ( empty( $valid['client_id'] ) ) {
			add_settings_error( 'client_id', 'no-client-id', __( 'Please input a Client ID before authorizing.', 'resp_github_plugin_updater' ), 'error' );
		}
		if ( empty( $valid['client_secret'] ) ) {
			add_settings_error( 'client_secret', 'no-client-secret', __( 'Please input a Client Secret before authorizing.', 'resp_github_plugin_updater' ), 'error' );
		}
	...
	function filter_plugin_actions( $links ) {
		$settings_link = '<a href="plugins.php?page=resp_github-updater">' . __( 'Setup', 'resp_github_plugin_updater' ) . '</a>';
...
?>
		<div class="wrap respghupdate-admin">

			<div class="head-wrap">
				<?php screen_icon( 'plugins' ); ?>
				<h2><?php _e( 'Setup Response Github Updates' , 'resp_github_plugin_updater' ); ?></h2>
			</div>

			<div class="postbox-container primary">
				<form method="post" id="respghupdate" action="options.php">
					<?php
		settings_errors();
		settings_fields( 'respghupdate' ); // includes nonce
		do_settings_sections( 'resp_github-updater' );
?>
				</form>
			</div>
	...
	public function maybe_authorize() {
		...
		$redirect_uri = urlencode( admin_url( 'admin-ajax.php?action=set_resp_github_oauth_key' ) );

		// Send user to Response Github for account authorization
	...
	public function ajax_set_resp_github_oauth_key() {
		$gh = get_option( 'respghupdate' );

		$query = admin_url( 'Response-Core-Plugin.php' );
		$query = add_query_arg( array( 'page' => 'resp_github-updater' ), $query );
		...
		if ( isset( $_GET['code'] ) ) {
			...
			if ( !empty( $access_token ) ) {
				...
				update_option( 'respghupdate', $gh );
			}

			wp_redirect( admin_url( 'plugins.php?page=resp_github-updater' ) );
			exit;
