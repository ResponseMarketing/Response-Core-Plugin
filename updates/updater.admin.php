 ...
 *
 * @see /wp-admin/plugins.php?page=response-updater
 */
class WPGitHubUpdaterSetup {
	...
	function __construct() {
		...
		$this->plugin_file = RP_GITHUB_PLUGIN_NAME;
		...
		add_action( 'wp_ajax_set_response_oauth_key', array( $this, 'ajax_set_response_oauth_key' ) );
	}
	...
	function add_page() {
		if ( current_user_can ( 'manage_options' ) ) {
			$this->options_page_hookname = add_submenu_page( 'response-core', __( '*RCP Updates', 'response_plugin_updater' ), __( '*RCP Updates', 'response_plugin_updater' ), 'manage_options', 'response-updater', array( $this, 'admin_page' ) );
		}
	}
	...
	public function settings_fields() {
		register_setting( 'responseupdate', 'responseupdate', array( $this, 'settings_validate' ) );

		// Sections: ID, Label, Description callback, Page ID
		add_settings_section( 'responseupdate_private', 'Private Repositories', array( $this, 'private_description' ), 'response-updater' );

		// Private Repo Fields: ID, Label, Display callback, Menu page slug, Form section, callback arguements
		add_settings_field(
			'client_id', 'Client ID', array( $this, 'input_field' ), 'response-updater', 'responseupdate_private',
			array(
				'id' => 'client_id',
				'type' => 'text',
				'description' => '',
		...
		add_settings_field(
			'client_secret', 'Client Secret', array( $this, 'input_field' ), 'response-updater', 'responseupdate_private',
			array(
				'id' => 'client_secret',
				'type' => 'text',
				'description' => '',
		...
		add_settings_field(
			'access_token', 'Access Token', array( $this, 'token_field' ), 'response-updater', 'responseupdate_private',
			array(
				'id' => 'access_token',
			)
		);
		add_settings_field(
			'gh_authorize', '<p class="submit"><input class="button-primary" type="submit" value="'.__( 'Authorize with Response', 'response_plugin_updater' ).'" /></p>', null, 'response-updater', 'responseupdate_private', null
		);
...
?>
		...
		<ol>
			...
			<li><a href="javascript:document.forms['responseupdate'].submit();">Authorize with Response</a>.</li>
		</ol>
		<?php
	}
...
?>
	...
	public function token_field( $args ) {
		...
		if ( empty( $value ) ) {
?>
			<p>Input Client ID and Client Secret, then <a href="javascript:document.forms['responseupdate'].submit();">Authorize with Response</a>.</p>
			<input value="<?php esc_attr_e( $value )?>" name="<?php esc_attr_e( $id ) ?>" id="<?php esc_attr_e( $id ) ?>" type="hidden" />
			<?php
		}else {
?>
			<input value="<?php esc_attr_e( $value )?>" name="<?php esc_attr_e( $id ) ?>" id="<?php esc_attr_e( $id ) ?>" type="text" class="regular-text" />
			<p>Add to the <strong>$update_config</strong> array: <code>'access_token' => '<?php echo $value ?>',</code>
			<?php
		}
?>
		<?php
	...
	public function settings_validate( $input ) {
		...
		if ( empty( $valid['client_id'] ) ) {
			add_settings_error( 'client_id', 'no-client-id', __( 'Please input a Client ID before authorizing.', 'response_plugin_updater' ), 'error' );
		}
		if ( empty( $valid['client_secret'] ) ) {
			add_settings_error( 'client_secret', 'no-client-secret', __( 'Please input a Client Secret before authorizing.', 'response_plugin_updater' ), 'error' );
		}
...
?>
		<div class="wrap responseupdate-admin">

			<div class="head-wrap">
				<?php screen_icon( 'plugins' ); ?>
				<h2><?php _e( 'Setup Response Updates' , 'response_plugin_updater' ); ?></h2>
			</div>

			<div class="postbox-container primary">
				<form method="post" id="responseupdate" action="options.php">
					<?php
		settings_errors();
		settings_fields( 'responseupdate' ); // includes nonce
		do_settings_sections( 'response-updater' );
?>
				</form>
			</div>
	...
	public function maybe_authorize() {
		...
		$redirect_uri = urlencode( admin_url( 'admin-ajax.php?action=set_response_oauth_key' ) );

		// Send user to Response for account authorization
	...
	public function ajax_set_response_oauth_key() {
		$gh = get_option( 'responseupdate' );

		$query = admin_url( 'Response-Core-Plugin.php' );
		$query = add_query_arg( array( 'page' => 'response-updater' ), $query );
		...
		if ( isset( $_GET['code'] ) ) {
			...
			if ( !empty( $access_token ) ) {
				...
				update_option( 'responseupdate', $gh );
			}

			wp_redirect( admin_url( 'plugins.php?page=response-updater' ) );
			exit;
